...
# Part ONE
def roundOutcome(pair_value):
    # A and X are ROCK, B and Y are PAPER, C and Z are SCISSOR
    outcomes_map = {
        "A X": 3 + 1, "A Y": 6 + 2, "A Z": 0 + 3,
        "B X": 0 + 1, "B Y": 3 + 2, "B Z": 6 + 3,
        "C X": 6 + 1, "C Y": 0 + 2, "C Z": 3 + 3,
    }
    return outcomes_map[pair_value]


# Part TWO
def riggedOutcome(pair_value):
    # X for losing, Y is to draw, Z is to win
    outcomes_map = {
...
if __name__ == "__main__":
    with (open("strategy_guide.csv", "r") as csv_file):
        input_file = csv.reader(csv_file)
        result = 0
        result_rigged = 0
        for game_round in input_file:
            result += roundOutcome(game_round[0])
            result_rigged += riggedOutcome(game_round[0])
        print("Initial strategy value: {0}".format(result))
        print("Rigged strategy value: {0}".format(result_rigged))
