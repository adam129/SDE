...
One Elf has the important job of loading all the rucksacks with supplies for the jungle journey. Unfortunately,
...
def rucksackSearch(rucksack_items):
    index = int(len(rucksack_items)/2)
    compartment_one = "[{0}]".format(rucksack_items[0: index])
    compartment_two = rucksack_items[index:]
...
def itemPriority(item):  # ord() and char() transform letter to number and viceversa
    if ord(item) >= 97:
        return ord(item) - 96
    elif 65 <= ord(item) <= 90:
        return ord(item) - (64 - 26)  # - 64 > A = 1 but A should be 27 so + 26 is needed
    else:
        print("ERROR")
